...
<form method="post" action="index.php?page=cart">
	...
	<table>
		...
		<form class="col-md-4 col-sm-offset-4 text-center" style="margin: 2%;background-color: #e3e8ef;border: 1px #e3e8ef;border-radius: 5%; opacity:0.9;filter: alpha(opacity=60);text-align: center;margin-left: 35%;padding-top: 2%;padding-bottom: 2%;box-shadow: 5px 10px #989ba0;margin-top: 4%" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]);?>" method="post" >
			  ...
			  <div class="form-group">
				<label for="adrs" style="font-weight: bold;color: #000000;">Email ID:</label>
				<input type="email" class="form-control" id="adrs" name="emailid" style="width:50%;margin-left: 24%;margin-top:-35px;" required>
			  </div>
			  <div class="form-group center">
				    <label>Region:</label>
                    
                    <select name="region">
					    <option value="Uttara">Mumbai</option>
			  ...
			  <div class="form-group">
				<label for="adrs" style="font-weight: bold;color: #000000; font-family: Verdana, Geneva, Tahoma, sans-serif;">Only Cash On Delivary!!!</label>
			  </div>
		
				<!------------------------------------------------- Send Email ---------------------------------------------------->				
				<?php
						// Place use statements at the beginning of the file
						...
						if ($_SERVER["REQUEST_METHOD"] == "POST") {
							...
							try {
								...
								$mail->SMTPAuth = true;
								$mail->Username = 'biradarrutuja125@gmail.com'; // Update with your Gmail username
								$mail->Password = 'rsmajhfnkhgoiiyh'; // Update with your Gmail password
								$mail->SMTPSecure = PHPMailer::ENCRYPTION_SMTPS;
								$mail->Port = 465;
								$mail->setFrom($_POST['emailid'], 'Customer');
								$mail->addAddress('biradarrutuja125@gmail.com', 'Miss. Rutu');
								$mail->isHTML(true);
								$mail->Subject = 'Medical HelpLine';
								$mail->Body = 'Customer Resquesed For Medicine...';
								$mail->AltBody = 'No Comments';
								$mail->send();
		...
		<?php
			...
			//$sql2="insert into medicineorder(medicinename) values('".$row['medName']."')";
              ...
              /* $result2= mysqli_query($conn,$sql2)or die(mysqli_error($conn));
               ...
               $sql1="insert into medicineorder(ordercusname,orderphone,orderaddress,emil,medicinename,medprice,totalprice,medquantity,orderregion) values('".$cuName."', '".$cuMobile."','".$cuAddress."','".$cuemail."','".$row['medName']."','".$row['price']."','".$totalprice."','".$_SESSION['cart'][$row['medId']]['quantity']."', '".$_POST['region']."')";
	...
	<br>
		...
		<button type="submit" onclick="myFunction()" class="btn btn-default" name="submit">Confirm Order</button>
