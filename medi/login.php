                            ...
                            else
                            {
							   ...
							   // session_start(); // Start the session
							    ...
							    if ($_SERVER["REQUEST_METHOD"] == "POST") {   
							        ...
							        $stmt = mysqli_prepare($con, "SELECT * FROM customerlogin WHERE cuUserName=? AND cuPassword=?");
							        ...
							        // If not a customer, check pharmacy login
							        $stmt = mysqli_prepare($con, "SELECT * FROM pharmacylogin WHERE phUserName=? AND phPassword=?");
							        mysqli_stmt_bind_param($stmt, 'ss', $username, $password);
							        mysqli_stmt_execute($stmt);
							        $result = mysqli_stmt_get_result($stmt);
							        $row = mysqli_fetch_assoc($result);
							        ...
							        // If not a pharmacy, check admin login
							        $stmt = mysqli_prepare($con, "SELECT * FROM admin WHERE aUserName=? AND aPassword=?");
							        mysqli_stmt_bind_param($stmt, 'ss', $username, $password);
							        mysqli_stmt_execute($stmt);
							        $result = mysqli_stmt_get_result($stmt);
							        $row = mysqli_fetch_assoc($result);
							        ...
							        // If not an admin, check delivery login
							        $stmt = mysqli_prepare($con, "SELECT * FROM delivery WHERE deName=? AND dePass=?");
							        mysqli_stmt_bind_param($stmt, 'ss', $username, $password);
							        mysqli_stmt_execute($stmt);
							        $result = mysqli_stmt_get_result($stmt);
							        $row = mysqli_fetch_assoc($result);
